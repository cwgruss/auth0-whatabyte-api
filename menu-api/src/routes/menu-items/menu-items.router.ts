...
import { MenuItemPermissions } from "../../core/menu-item/menu-item.permissions";
...
menuItemsRouter.post(
  "/",
  checkPermissions(MenuItemPermissions.CreateItems),
...
menuItemsRouter.put(
  "/",
  checkPermissions(MenuItemPermissions.UpdateItems),
  async (req: Request, res: Response) => {
    try {
      const item: MenuItem = req.body.item;
...
menuItemsRouter.delete(
  "/:id",
  checkPermissions(MenuItemPermissions.DeleteItems),
  async (req: Request, res: Response) => {
    try {
      const id: number = parseInt(req.params.id, 10);
